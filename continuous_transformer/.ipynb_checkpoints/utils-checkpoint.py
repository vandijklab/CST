...
#     def __call__(self, val_loss, model, string_name, path_to_save):

#         score = -val_loss

#         if self.best_score is None:
#             self.best_score = score
#             self.save_checkpoint(val_loss, model, string_name, path_to_save)
...
# #         elif score == -1000
# #             self.counter = 0
# #             print(f'Resetting EarlyStopping (counter: {self.counter})')
#         else:
#             self.best_score = score
#             self.save_checkpoint(val_loss, model, string_name, path_to_save)
#             self.counter = 0

#     def save_checkpoint(self, val_loss, model, string_name, path_to_save):
#         '''Saves model when validation loss decrease.'''
#         if self.verbose:
#             print(f'Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
#         torch.save(model.state_dict(), os.path.join(path_to_save,'checkpoint_' + string_name + '.pt'))
